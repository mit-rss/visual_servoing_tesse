...
The `HomographyConverter` starts by picking four arbitrary (x, y) points in the ground plane. It projects these points into the camera frame, using the known intrinsic and extrinsic properties of the camera. Then it calls to an OpenCV function to compute the homography matrix, which performs the reverse projection from pixel coordinates (u, v) back into ground plane points (x, y). 
